...
export default function Home() {
  return <PageFrame>
    ...
    <section className="section category-section"><div className="container"><SectionHeading eyebrow="Browse by category" title="A little bit of everything" href="/categories" label="View all" /><div className="category-grid">{categories.map((item) => <Link className="category-card" key={item.name} href={`/shop?category=${encodeURIComponent(item.name)}`}><span className={`category-icon ${item.color}`}>{item.icon}</span><strong>{item.name}</strong><small>{item.count}</small><ArrowRight size={16} /></Link>)}</div></div></section>
